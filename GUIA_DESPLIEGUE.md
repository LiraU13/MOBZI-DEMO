...
4.  Configuración:
    - **Root Directory**: `MobziGuest/backend`
    - **Build Command**: `npm install && npm run build`
    - **Start Command**: `npm run start`
    - **Environment Variables** (Añadir estas variables):
      - `DB_TYPE`: `postgresql`
      - `DB_POSTGRES_HOST`: (host de Neon, sin `postgres://` ni puerto)
      - `DB_POSTGRES_PORT`: `5432`
      - `DB_POSTGRES_USER`: (usuario de Neon)
      - `DB_POSTGRES_PASSWORD`: (contraseña de Neon)
      - `DB_POSTGRES_DATABASE`: `neondb` (o el nombre que te de Neon)
...
3.  Configuración:
    - **Root Directory**: `MobziGuest/frontend` (Editalo si no lo detecta).
